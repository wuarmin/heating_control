...
def test_init():
  rule = PumpControlRule(nominal_temperature=45, start_temperature=30, time_slots=4, time_controls=[
    TimeControl(name="Night",   check_interval=3600, outdoor_max=15, start_at=time(22), end_at=time(3, 59)),
    TimeControl(name="Morning", check_interval=3600, outdoor_max=15, start_at=time(4),  end_at=time(8)),
  ])
...
def test_next_schedule_returns_none_because_subsequent_schedule_will_start_before_current_will_be_applied(pump):
  ...
  next_schedule = rule.next_schedule(pump=pump('OG'), current_date_time=datetime(1989, 12, 24, 3, 55, 59, 000000))
  assert next_schedule is None

def test_next_schedule_returns_a_schedule_which_is_in_current_time_control( pump):
  rule = PumpControlRule(nominal_temperature=45, start_temperature=30, time_slots=4, time_controls=[
    TimeControl(name="Night", check_interval=8800, outdoor_max=15, start_at=time(22), end_at=time(21, 59)),
  ])
  next_schedule = rule.next_schedule(pump=pump('OG'), current_date_time=datetime(1989, 12, 24, 23, 30, 59, 000000))
...
def test_starts_at_current_time_control_but_ends_with_start_of_subsequent_control_same_day(pump):
  rule = PumpControlRule(nominal_temperature=45, start_temperature=30, time_slots=4, time_controls=[
    TimeControl(name="Night",   check_interval=3600, outdoor_max=15, start_at=time(23),     end_at=time(23, 50)),
    TimeControl(name="Morning", check_interval=3600, outdoor_max=15, start_at=time(23, 51), end_at=time(8)),
  ])
...
def test_starts_at_current_time_control_but_ends_with_start_of_subsequent_control_next_day(pump):
  rule = PumpControlRule(nominal_temperature=45, start_temperature=30, time_slots=4, time_controls=[
    TimeControl(name="Night",   check_interval=3600, outdoor_max=15, start_at=time(23),   end_at=time(0, 5)),
    TimeControl(name="Morning", check_interval=3600, outdoor_max=15, start_at=time(0, 6), end_at=time(8)),
  ])
